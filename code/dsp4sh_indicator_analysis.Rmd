---
title: "DSP4SH - Soil Health Indicator Variability and Management Sensitivity"
...
```{r Table 2 summary of random effects models}
...
Calculate random-effects model with land use as a moderator variable, generate table of results, and generate table of indicators for which land use is a significant moderator variable:
...
Plot relationship between land use and indicator variables for which it is a significant predictor:
...
```{r table s4 significant moderator variables}
...
```{r indicator boxplots, include=FALSE}

indicator_plots <- purrr::map(.x = indicators,
                  .f = ~{
                    surf_long %>% 
                      filter(indicator == .x) %>%
                      ggplot(aes(x=project, y=value, fill=label)) + 
                      geom_boxplot() +
                      labs(title=glue::glue({unique(filter(indicator_labs_df, indicator==.x)$label)})) + 
                      scale_fill_manual(values=c("#FED789FF","#72874EFF","#476F84FF")) + 
                      theme_katy()
...
summary(lm(soil_respiration ~ (project+label)^2, data=surf))
```

```{r save rdata for ncss poster figures, include=FALSE}
# save(surf,
    # imp_pca,
    # multi_ref_featimp_df,
    # es_rma_summary,
    # shape_lmer_df,
    # file=here::here("data_processed", "data_for_ncss.RData"))
```